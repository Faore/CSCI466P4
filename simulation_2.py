...
if __name__ == '__main__':
    ...
    link_layer.add_link(link_2.Link(host_2, 0, router_a, 3))
    ...
    link_layer.add_link(link_2.Link(router_d, 1, host_3, 0))
    ...
    #Give the routers enough time to build their tables.
    sleep(simulation_time)
    #create some send events    
    for i in range(1):
        host_1.udt_send(3, 'Sample client data %d' % i)
        host_3.udt_send(1, 'Sample server reply %d' % i)
